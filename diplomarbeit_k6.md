...
Das Konzept wurde im Projekt vollumfänglich nach dem "Principle of Least Privilege" (Prinzip der minimalen Rechtevergabe) ausgearbeitet. Ein User (`user`) darf exklusiv Methoden der API aufrufen, welche den Filter `user_id == req.user.id` anwenden (z. B. das Laden *seiner eigenen* Stempelzeiten). Die Modifikation von Fremddaten (wie dem Dienstplan oder den Dienstverträgen in der User-Verwaltung) ist in einem Sandbox-Bereich gekapselt, in den keine unterprivilegierte Rolleneinordnung vorstoßen kann. 
