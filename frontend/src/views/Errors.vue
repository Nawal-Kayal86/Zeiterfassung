...
const openEditModal = async (log) => {
  ...
  try {
    const res = await api.get(
      `/workSessions?userId=${log.user_id._id}&startDate=${log.violation_date}&endDate=${log.violation_date}`,
    );
    if (res.data.length > 0) {
      const s = res.data[0];
      if (s.start)
        form.value.start_time = new Date(s.start).toTimeString().slice(0, 5);
      if (s.end)
        form.value.end_time = new Date(s.end).toTimeString().slice(0, 5);
    }
  } catch (err) {
