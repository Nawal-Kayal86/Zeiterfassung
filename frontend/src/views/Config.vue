  ...
  <div class="container-fluid py-4 px-md-5">
    <div class="d-flex justify-content-between align-items-center mb-4">
      <div>
        ...
        <p class="text-muted">
          Verwalten Sie Feiertage und Betriebsferien für das System.
        </p>
      </div>
      <div
        class="bg-white p-2 rounded-3 shadow-sm d-flex align-items-center gap-3 border"
      >
        <label class="mb-0 fw-bold small text-muted text-uppercase"
          >Jahr wählen:</label
        >
        <select
          v-model="year"
    ...
    <div class="row g-4">
      ...
      <div class="col-lg-6">
        <div class="card shadow-sm border-0 h-100 bg-white">
          ...
          <div class="card-body">
            <div
              class="holiday-list custom-scrollbar pe-2"
            ...
            >
              <div
                ...
                :class="{
                  'border-danger bg-danger-soft': changedHolidays.includes(
                    h.date,
                  ),
                }"
              >
                ...
                />
                <button
                  class="btn btn-sm text-danger hover-bg-danger-soft border-0"
            ...
            <button
              class="btn btn-indigo-soft w-100 mt-3 fw-bold"
              @click="addHoliday"
            >
      ...
      <div class="col-lg-6">
        <div class="card shadow-sm border-0 h-100 bg-white">
          ...
          <div class="card-body">
            <div
              class="ferien-list custom-scrollbar pe-2"
              style="max-height: 500px; overflow-y: auto"
            >
              <div
                v-for="(f, index) in ferien"
                :key="index"
              ...
              >
                <div class="d-flex justify-content-between mb-2">
                  <input
                    v-model="f.name"
                    class="form-control form-control-sm border-0 bg-transparent fw-bold fs-6 p-0"
                    placeholder="Ferien Bezeichnung"
                  />
                  <button
                    class="btn btn-sm text-danger border-0 p-0"
                    @click="removeFerien(index)"
                  >
                    <i class="bi bi-trash3"></i>
                  </button>
                </div>
    ...
    <div
      class="fixed-bottom p-4 d-flex justify-content-end pointer-events-none"
    >
      <button
        class="btn btn-indigo shadow-lg px-5 py-3 rounded-pill fw-bold pointer-events-auto transition-3d"
        @click="saveData"
      >
        <i class="bi bi-cloud-check-fill fs-5 me-2"></i> Alles Speichern /
        Updaten
      </button>
    </div>
  </div>
...
async function fetchFromInternet() {
  try {
    const res = await fetch(
      `https://date.nager.at/api/v3/PublicHolidays/${year.value}/AT`,
    );
    ...
    if (changedHolidays.value.length) {
      alert(
        `ℹ️ Es gibt ${changedHolidays.value.length} neue oder geänderte Feiertage.`,
      );
    } else {
